...
// ---------------- [ARQUIVO DE TESTE] Fonte da câmera: webcam USB via Raspberry Pi na rede ----------------
// Esta é a versão de TESTE — terminal.js (o totem de verdade) continua
// intocado, com USAR_WEBCAM_USB = false, exatamente como está funcionando
// hoje. Só promova esta mudança pra lá depois de validar na prática.
//
// Histórico até 11/07/2026: tentamos várias vezes plugar a webcam USB direto
// no TABLET que mostra o totem (primeiro via app terceiro + proxy Termux,
// depois via app Android nativo com plugin UVC). Sempre esbarrou no mesmo
// problema físico: a porta USB-OTG do tablet não sustenta a energia que a
// webcam pede durante o streaming, e ela cai sozinha a cada poucos segundos
// ("connected=false/no-power" no log do Android). Isso fez a gente voltar
// pra câmera embutida do tablet (getUserMedia) por um tempo — ver
// totem-app-scaffold/totem-app/ pro trabalho arquivado dessa fase.
//
// 2026-07-19 — NOVA ARQUITETURA (em teste): a tela do totem vai trocar pra
// um dispositivo sem câmera embutida e sem toque (TV), então voltamos a
// precisar de uma webcam externa — só que desta vez o problema de energia
// do tablet não se aplica: um Raspberry Pi (porta USB de verdade, com
// energia estável) fica FISICAMENTE perto da catraca com a webcam plugada
// nele, e publica o vídeo pela rede local como stream MJPEG, já com
// cabeçalho CORS liberado (ver scripts/totem/webcam_raspberry.py — tem
// instruções completas de instalação e de deixar rodando sozinho no boot).
// A tela que mostra este arquivo de teste só precisa estar na mesma rede
// local e apontar pra esse endereço.
//
// Sem CORS a leitura de pixels via <canvas> (usada pra reconhecimento
// facial e leitura de QR) trava com erro de segurança do navegador, mesmo
// com a imagem aparecendo normal na tela — por isso o servidor do Pi já
// nasce com "Access-Control-Allow-Origin" liberado.
//
// TROQUE o endereço abaixo pelo IP de verdade do seu Raspberry Pi na rede
// (rode "hostname -I" nele pra descobrir). Configure uma RESERVA DE IP pra
// ele no roteador (por MAC address) — sem isso, se o roteador distribuir um
// IP diferente num reinício, este teste para de achar a câmera até alguém
// atualizar esta constante de novo.
const USAR_WEBCAM_USB = true;
const USB_WEBCAM_URL = 'http://192.168.0.50:9000/video'; // <- troque pelo IP fixo do Raspberry Pi
...
async function iniciarCamera(videoEl) {
  ...
  if (USAR_WEBCAM_USB) {
    // MJPEG (multipart/x-mixed-replace, o formato do stream do Raspberry Pi)
    // só é exibido de verdade por uma tag <img> — <video> não sabe decodificar
    // isso (não é um contêiner de vídeo, é uma sequência de imagens JPEG
    // amarradas). O HTML traz um <video> nesse lugar porque é o que o modo
    // getUserMedia (câmera embutida) usa; na primeira vez que esta função
    // roda em modo webcam USB, troca esse <video> por um <img> com o mesmo
    // id/classe (o CSS já estiliza "img.camera-feed" pra isso — mesmo tamanho
    // e espelhamento do <video>). Todo o resto do código sempre busca o
    // elemento de novo via getElementById (nunca guarda a referência antiga),
    // então continua funcionando sozinho depois da troca.
    let elemento = videoEl;
    if (elemento.tagName !== 'IMG') {
      const img = document.createElement('img');
      img.id = elemento.id;
      img.className = elemento.className;
    ...
    // crossOrigin = 'anonymous': pede a imagem em modo CORS. O Raspberry Pi
    // responde com Access-Control-Allow-Origin liberado, então isso faz o
    // canvas NÃO ficar "contaminado" — sem isso, drawImage/getImageData
    // (usados pra ler QR code e rosto) travariam com erro de segurança mesmo
    // com a imagem aparecendo normal na tela.
    ...
    await new Promise((resolve, reject) => {
      ...
      videoEl.onerror = () => reject(new Error(
        'Não foi possível conectar à webcam USB. Verifique se o Raspberry Pi está ligado, com scripts/totem/webcam_raspberry.py rodando, e se o endereço em USB_WEBCAM_URL (no topo deste arquivo) ainda aponta pro IP certo dele na rede.'
      ));
